...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'http://arran.no/birra/?Article=91'"/>
    <xsl:variable name="text_encoding" select="''"/>
    <xsl:variable name="title" select="'ÁRRANA BIRRA'"/>
    <!--
        Information of the first author: first name, last name,
        date or year of birth, nationality.
    -->
    <xsl:variable name="author1_fn" select="'Stefan'"/>
    <xsl:variable name="author1_ln" select="'Näslund'"/>
    <!-- m, f or unknown -->
    ...
    <xsl:variable name="parallels">
        ...
        <parallel_text xml:lang="nob" location="om_arran.html"/>
    ...
    <xsl:template match="p[parent::body][not(./em | ./span)][text()]">
        ...
        <xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
                ...
                <xsl:with-param name="target" select="'[1] Duodje= samisk begrep for samisk håndverk, kunsthåndverk og småindustri./Ønsker du å motta nyhetsbrev fra Árran?/Registrer inn din epostadresse under. Du kan også kontakte oss pr epost: poassta@arran.no eller telefon +47 757 75 100/Email Address/'"/>
